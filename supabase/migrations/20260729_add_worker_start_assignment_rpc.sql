...
CREATE OR REPLACE FUNCTION public.worker_start_assignment(p_asignacion_id uuid)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  ...
  IF NOT private.current_user_has_role(ARRAY['Trabajador']) THEN RAISE EXCEPTION 'An active Trabajador profile is required'; END IF;
  SELECT oa.estado INTO v_assignment_status
  FROM public.orden_asignaciones AS oa
  JOIN public.trabajadores AS t ON t.id = oa.trabajador_id
  WHERE oa.id = p_asignacion_id AND t.auth_user_id = auth.uid()
  FOR UPDATE OF oa;
  IF NOT FOUND THEN RAISE EXCEPTION 'The assignment does not belong to this worker'; END IF;
  IF v_assignment_status = 'pendiente' THEN
    UPDATE public.orden_asignaciones AS oa
    SET estado = 'en_progreso'
    WHERE oa.id = p_asignacion_id AND oa.estado = 'pendiente'
    RETURNING oa.estado INTO v_assignment_status;
  END IF;
  RETURN v_assignment_status;
END;
$$;

REVOKE EXECUTE ON FUNCTION public.worker_start_assignment(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.worker_start_assignment(uuid) TO authenticated, service_role;
COMMENT ON FUNCTION public.worker_start_assignment(uuid) IS
  'Starts only the authenticated active Trabajador pending assignment; completed and cancelled assignments remain unchanged.';
...
BEGIN
  ...
  -- A normal report starts only its selected pending assignment; it never reactivates other states.
  IF NOT p_completar_asignacion
     AND p_asignacion_id IS NOT NULL THEN
    UPDATE public.orden_asignaciones AS oa
    SET estado = 'en_progreso'
    WHERE oa.id = p_asignacion_id
      AND oa.orden_id = p_order_id
      AND oa.estado = 'pendiente';
  END IF;
