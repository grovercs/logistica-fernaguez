...
// load (e.g. Layout + RequireLiquidacionesEstadisticasAccess both using the hook).
const cache = new Map<string, { isAllowed: boolean; expiresAt: number }>();
const CACHE_TTL_MS = 5_000;

export function useLiquidacionesEstadisticasAccess() {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      const { error } = await supabase.rpc('get_liquidaciones_estadisticas_access');
      ...
      if (error) {
        ...
        if (code === 'FORBIDDEN' || code === 'AUTH_REQUIRED') {
          ...
          console.error('Liquidaciones estadísticas access check failed:', error);
          allowed = false;
        }
      }
