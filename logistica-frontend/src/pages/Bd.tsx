...
export default function Bd() {
  ...
  const fetchArchivados = async () => {
     ...
     const { data, error } = await supabase
        .from('ordenes')
        .select('*')
        .in('estado', ['Finalizada', 'Finalizado', 'Archivado'])
  ...
  const filteredArchivados = archivados.filter(item => 
    ...
    (item.id_legible || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
                                    ...
                                    {loading ? (
                                        ...
                                        filteredArchivados.map((obra) => (
                                            <tr key={obra.id} className={`border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors ${selectedArchivados.includes(obra.id) ? 'bg-primary/5 dark:bg-primary/10' : ''}`}>
                                                ...
                                                <td className="py-3 px-4 text-sm text-slate-600 dark:text-slate-400 font-medium">{new Date(obra.creado_en).toLocaleDateString('es-ES')}</td>
                                                ...
                                                <td className="py-3 px-4 text-sm text-slate-500 dark:text-slate-400 font-mono">{obra.poliza || 'N/A'}</td>
                                                <td className="py-3 px-4 text-center">
                                                    <span className={`inline-flex px-2 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
                                                        obra.estado === 'Finalizada' || obra.estado === 'Finalizado' 
