...
export default function BackupCenter() {
  ...
  const mediaChecks = (job: MediaBackupJob) => {
    const evidenceLocated = ['downloading', 'compressing', 'verifying', 'completed'].includes(job.estado);
    const evidenceCopied = ['compressing', 'verifying', 'completed'].includes(job.estado);
    const zipGenerated = job.estado === 'completed';
    return <div className="mt-2 space-y-1 text-[9px] font-bold uppercase tracking-wide text-slate-400">
      {evidenceLocated && <p>✓ Evidencias localizadas</p>}
      {evidenceCopied && <p>✓ {job.processed_items} / {job.total_items} EVIDENCIAS COPIADAS</p>}
      {job.estado === 'compressing' && <p>○ Generando ZIP...</p>}
      {job.estado === 'verifying' && <p>○ Preparando archivo final...</p>}
      {zipGenerated && <p>✓ ZIP generado</p>}
      {job.estado === 'completed' && <p>✓ Copia multimedia lista</p>}
    </div>;
  };
