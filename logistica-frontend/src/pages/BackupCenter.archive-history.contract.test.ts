...
assert.match(source, /if \(!reports\.length && isUuid\(order\.id_legible\)\)/);
assert.match(source, /reportes\.orden_id.*UUID foreign key/);
assert.match(source, /from\('perfiles'\)\.select\('id, nombre_completo'\)/);
assert.match(source, /if \(!profilesError\) profiles\?\.forEach/);
